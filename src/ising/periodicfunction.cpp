...
//        J_integral[it->first]=round(it->second );
        ...
        J_integral[it->first]=round(it->second / global_parameters.prec);
    }
...
//            cout<<string_tmp;
        ...
        else if(global_parameters.which_incomplete_solver=="CCLS_2016"||global_parameters.which_incomplete_solver=="CCEHC_2016")
        {
            string string_tmp;
            string_tmp="./"+global_parameters.which_incomplete_solver+" " + softer_filename ;
            result_now = exec(string_tmp);
        }
        else
        {
...
//                cout << "in periodic x_range, y_range, z_range "<<make_tuple(x_range,y_range,z_range);
                ...
                
                {
                    ...
                    if (global_parameters.Use_simulated_annealing_as_incomplete_solver){
//                        cout<<"\ncheck point, my J is: "<<endl;
//                        printmapfromsets(J);
                        spin_periodic_struct monte_spin_struct;
                        double monte_energy;
                        
                        global_parameters.monte_carlo_periodicity=periodicity_now;
                        
                        simulated_annealing(J,global_parameters,monte_energy,monte_spin_struct,constant);
                        
                        energy_tmp=monte_energy;
                        spin_tmp=monte_spin_struct.spin;
