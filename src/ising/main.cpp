...
//    std::string id="IS0";
    ...
    global_parameters.binary_x_min=0;
    global_parameters.binary_x_max=2;
    global_parameters.Use_simulated_annealing_as_incomplete_solver=false;
...
//    cout<<"\npositive_modulo(-4, 10)"<<positive_modulo(-4, 10);
    if (!scan_chemical_potential&&!global_parameters.do_monte_carlo&&!global_parameters.ternary_alg&&!global_parameters.ternary_debug&&!global_parameters.activate_large_cell_algo_binary) {
        ...
        cout<<"\n tell me what is upper bound: "<<upper_bound+constant<<" what is lower bound: "<<lower_bound+constant<<endl;
//<<" what is exact lower bound "<< exact_lowerbound+constant
...
//        cout<<"\n formation energy for 0 structure in the 1st method is "<< formation_energy_temp<< endl;
...
//        get_concentration_formation_energy_from_spin_and_J_in(make_tuple(1,0,1,0,0,1), mu, J_fixed_part, unit_cell_temp, constant, mu_constant, formation_energy_out, concentration_out);

//        cout<<"\n formation energy for 0 structure in the 2nd method is "<< formation_energy_out<<" and concentration is "<< concentration_out << endl;
...
//        cout<<"\n formation energy for 1 structure in the 1st method is "<< formation_energy_temp<< endl;
        unit_cell_temp.clear();
        for (map< set<tuple<int,int,int,int,int> >, double>::iterator it1=mu.begin(); it1!=mu.end(); it1++) {
            set<tuple<int,int,int,int,int> > set_temp=it1->first;
            for (set<tuple<int,int,int,int,int> >::iterator it2=set_temp.begin(); it2!=set_temp.end(); it2++) {
...
//        formation_energy_out=0 , concentration_out=0;
//        get_concentration_formation_energy_from_spin_and_J_in(make_tuple(1,0,1,0,0,1), mu, J_fixed_part, unit_cell_temp, constant, mu_constant, formation_energy_out, concentration_out);
//        
//        cout<<"\n formation energy for 1 structure in the 2nd method is "<< formation_energy_out<<" and concentration is "<< concentration_out << endl;
        ...
        while (!to_scan_line.empty()) {
            ...
            double conc_1=concentration_vector[the_line_to_check[0]];
            double conc_2=concentration_vector[the_line_to_check[1]];
            ...
            cout<< "conc_1 is "<<conc_1<<endl;
            cout<< "conc_2 is "<<conc_2<<endl;
            cout<< "global_parameters.binary_x_min is "<<global_parameters.binary_x_min<<endl;
            cout<< "global_parameters.binary_x_max is "<< global_parameters.binary_x_max<<endl;
            cout<<"(conc_1<global_parameters.binary_x_min-1e-5 && conc_2<global_parameters.binary_x_min-1e-5) is "<<(conc_1<global_parameters.binary_x_min-1e-5 && conc_2<global_parameters.binary_x_min-1e-5)<<endl;
            cout<< "(conc_1>global_parameters.binary_x_max+1e-5 && conc_2>global_parameters.binary_x_max+1e-5 ) is "<<(conc_1>global_parameters.binary_x_max+1e-5 && conc_2>global_parameters.binary_x_max+1e-5 )<<endl;
            
            
            
            if ((conc_1<global_parameters.binary_x_min-1e-5 && conc_2<global_parameters.binary_x_min-1e-5) ||(conc_1>global_parameters.binary_x_max+1e-5 && conc_2>global_parameters.binary_x_max+1e-5 ))
            {
                cout<<"\nthis facet is not interested skip";
...
void read_from_file(std::string &id,
        ...
        for (vector<string>::iterator it=J_config_line.begin(); it!=J_config_line.end(); it++) {
            ...
            else if (equal_left=="binary_x_min"){
                global_parameters.binary_x_min=lexical_cast<double>(equal_right);
            }
            else if (equal_left=="binary_x_max"){
                global_parameters.binary_x_max=lexical_cast<double>(equal_right);
            }
            else if (equal_left=="Use_simulated_annealing_as_incomplete_solver"){
                global_parameters.Use_simulated_annealing_as_incomplete_solver=lexical_cast<bool>(equal_right);
            }
        }
...
//    {
//        cout<<"\n debug oaisojancidy8y91 let me check the sum of all digits in J_negative1_positive1"<<endl;
//    
//    double sum_tmp=constant_term;
//    for(map< set<tuple<int,int,int,int,int> >, double>::iterator it1=J_negative1_positive1.begin();it1!=J_negative1_positive1.end();it1++ ){
//        sum_tmp+=it1->second;
//        
//    }
//    
//    cout<<"\n debug vicushdifbiu921 the sum is "<<sum_tmp<<endl;
//
//    }
...
//    {
//    cout<<"\n debug oaisojancidy8y91 after translations let me check the sum of all digits in J"<<endl;
//    
//    double sum_tmp=constant_term;
//    for(map< set<tuple<int,int,int,int,int> >, double>::iterator it1=J_negative1_positive1.begin();it1!=J_negative1_positive1.end();it1++ ){
//        sum_tmp+=it1->second;
