...
def parse_FCLUST(fclust_file, coords, sites, species_index, eci, cluster):
    if cluster[0] != 0:
        print("Constant {}\n".format(0))
    else:
        print("Constant {}\n".format(eci[0]))
    nclus = 0
    with open(fclust_file, 'r') as fp:
...
if (__name__ == "__main__"):
    ...
    parser.add_argument(
        "--FCLUST", "-f",
        help="Path to a CASM FCLUST file (default: FCLUST).",
        default="FCLUST")
    ...
    convert(args.PRIM, args.FCLUST, args.eci_out)
